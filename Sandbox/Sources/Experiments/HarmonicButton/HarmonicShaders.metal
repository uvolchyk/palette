...
//  uv -= float2(0.5, 0.5);
//
////  float am = cos(uv.x * 3.0) * amplitude * 0.2;
////  float offset = sin(uv.x * 12.0 + time) * am * 0.1;
//  float frequency = mix(3.0, 12.0, monochrome);
//  float width = mix(0.6, 0.9, monochrome);
//  float intensity = mix(0.02, 0.01, monochrome);
//
//  float3 finalColor = float3(0.0);
//
//  for (float i = 0.0; i < count; i++) {
//    float phase = time + i * M_PI_F / count;
//    float dist = glow(harmonicSDF(uv, am, offset, frequency, phase), width, intensity);
//
//    float3 waveColor = mix(float3(1.0), getColor(i), monochrome);
//
//    finalColor += waveColor * dist;
//  }
//
//  return half4(half3(finalColor), 1.0);
//}
