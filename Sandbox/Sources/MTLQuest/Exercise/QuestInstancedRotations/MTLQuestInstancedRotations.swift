//
//  MTLQuestInstancedRotations.swift
//  Sandbox
//
...
struct MTLQuestInstancedRotations: UIViewRepresentable {
...
extension MTLQuestInstancedRotations {
  @MainActor
  ...
  final class Coordinator {
    ...
    private let instanceCount: Int = 20

    private let gridColumns = 5
    private let gridRows = 4
    private let gridSpacing: Float = 10
    ...
    lazy var renderer: MTLQuestRenderer = {
      MTLQuestRenderer(
        ...
        for i in 0..<instanceTransforms.count {
          ...
          //          let angle = automaticAngle + Float(i) * (.pi * 2 / Float(instanceTransforms.count))
          //
          //          let axis: SIMD3<Float> = (i % 3 == 0) ? SIMD3<Float>(0,1,0) : ((i % 3 == 1) ? SIMD3<Float>(1,0,0) : SIMD3<Float>(0,0,1))
          //
          //          t.rotation = simd_quatf(angle: angle, axis: axis)
    ...
    init(
      ...
      self.library = .init(
        ...
        namespace: String(describing: MTLQuestInstancedRotations.self)
      )
