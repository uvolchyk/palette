...
extension MTLQuestTen {
  ...
  final class Coordinator {
    ...
    private var cameraYaw: Float = 0.0 // left/right (around Y)
    private var cameraPitch: Float = 0.0 // up/down (around X)
    ...
    lazy var renderer: MTLQuestRenderer = {
      MTLQuestRenderer(
        ...
        let cx = cos(cameraPitch) * sin(cameraYaw)
        let cy = sin(cameraPitch)
        let cz = cos(cameraPitch) * cos(cameraYaw)
    ...
    @objc func handleRotate(_ gesture: UIPanGestureRecognizer) {
      ...
      cameraYaw += Float(translation.x) * yawSensitivity
      cameraPitch += Float(translation.y) * pitchSensitivity
      cameraPitch = cameraPitch.clamp(
        minValue: -.pi/2,
        maxValue: .pi/2
